...
for private_key in keys_list:
    ...
    try:
        ...
        gasLimit = 21000
        komissia = maxFeePerGas * gasLimit * config.gas_kef
        komissia_mosta = web3.to_wei(0.0015, 'ether') #orbiter фиксированная комиссия

        if config.bridge_all_money:
            amount = int(balance - komissia)# сколько хотим получить на выходе
        else:
            amount_decimal = random.randint(int(config.bridge_min*100000000),int(config.bridge_max*100000000))/100000000
            amount = web3.to_wei(amount_decimal, 'ether') 

        value = int( amount + komissia_mosta)  # сумма которая будет отправлена в самой транзакции. в нее войдет комиссия моста
        value = value // 10000 * 10000 + 9019
