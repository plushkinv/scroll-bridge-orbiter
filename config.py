...
minimal_need_balance = 0.007 # минимальный баланс на кошельке который должен быть чтобы начать  с ним работу
bridge_all_money = False # True  если хотите перевести максимум из доступной суммы. False - будут действовать параметры ниже
bridge_min = 0.005  # ETH  в орбитре минмум   0.005 к этому прибавится комиссия моста 0,0015
bridge_max = 0.007  # ETH
...
max_gas_price = 21 #максимальная цена газа в сети эфир при которой выполняются транзакции . если будет больше то скрипт будет ждать когда цена снизится
